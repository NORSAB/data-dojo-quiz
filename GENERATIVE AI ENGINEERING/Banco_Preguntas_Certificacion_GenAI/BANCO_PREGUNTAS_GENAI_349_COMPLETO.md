...
## Banco Maestro Completo de 349 Preguntas (Dominios 1 al 6)

> **Total de Preguntas en esta sección**: 349
...
### Pregunta NaN: An engineer is chunking structured Markdown technical documentation with nested headers (# H1, ## H2, ### H3). Which chunking strategy preserves semantic context best?
...
### Pregunta NaN: Why is chunk overlap (e.g. 50-100 tokens) essential when chunking long narrative text for RAG?
...
### Pregunta NaN: When embedding chunks using a model with a maximum context length of 512 tokens (e.g. `bge-small-en-v1.5`), how should the chunk size be configured?
...
### Pregunta NaN: For source code repositories containing Python files, which specialized text splitter in LangChain should be used?
...
### Pregunta NaN: In semantic chunking (embedding-based chunking), how are chunk boundaries determined?
...
### Pregunta NaN: What is a major risk of setting the chunk size too small (e.g. 20 tokens)?
...
### Pregunta NaN: What is a major downside of setting chunk size too large (e.g. 4000 tokens) in a standard RAG system?
...
### Pregunta NaN: When processing PDF forms with tables, why does naive character chunking often fail?
...
### Pregunta NaN: When parsing raw HTML scraped from public web pages, which content should be filtered out before chunking and embedding for RAG?
...
### Pregunta NaN: How do repeated legal disclaimers and copyright notices at the bottom of every document page affect Vector Search retrieval if left unfiltered?
...
### Pregunta NaN: What technique is recommended to clean OCR text containing corrupted scanning artifacts (e.g. `Th1s |s 4 t3xt`) before embedding?
...
### Pregunta NaN: A source repository of PDFs contains hundreds of duplicate revised versions of the same policy manual. What deduplication strategy should be applied?
...
### Pregunta NaN: When processing source documents containing sensitive PII (Social Security Numbers, credit card numbers) that is irrelevant to the RAG knowledge base, what should be done?
...
### Pregunta NaN: Why should non-informative content such as page numbers, header breadcrumbs, and 'Page X of Y' strings be stripped during document extraction?
...
### Pregunta NaN: In Databricks Lakehouse, which tool can automatically apply data quality expectations and drop corrupted rows during ingestion pipelines?
...
### Pregunta NaN: Which Python package is best suited for extracting clean text, layouts, and bounding boxes from native digital PDF documents in Python?
...
### Pregunta NaN: For extracting structured tables and text from Microsoft Word `.docx` documents, which Python library should be imported?
...
### Pregunta NaN: When processing scanned image-based PDFs requiring Optical Character Recognition (OCR), which library or service is standard in Python / Databricks?
...
### Pregunta NaN: Which Python package parses raw HTML documents and enables DOM traversal using CSS selectors or tag navigation?
...
### Pregunta NaN: Which Python library handles complex multi-format document conversions (PDF, PPTX, DOCX, XLSX, HTML, Images) to Markdown in Microsoft's open-source ecosystem?
...
### Pregunta NaN: For extracting tabular sheets and cell data from Excel `.xlsx` files into structured Python objects or DataFrames, which package is primary?
...
### Pregunta NaN: Which comprehensive document parsing framework natively integrates with LangChain for handling unstructured multi-modal documents with OCR, table parsing, and layout detection?
...
### Pregunta NaN: What is the correct logical sequence of operations to process raw documents from cloud object storage and write indexed chunks into a Unity Catalog Delta table?
...
### Pregunta NaN: Why must Change Data Feed (CDF) be enabled on a Delta table (`delta.enableChangeDataFeed = true`) when used as a source for Mosaic AI Vector Search?
...
### Pregunta NaN: Which SQL command enables Change Data Feed on an existing Delta table in Unity Catalog?
...
### Pregunta NaN: What primary key requirement exists for Delta tables intended to sync with Mosaic AI Vector Search?
...
### Pregunta NaN: When writing chunked records to Delta Lake in PySpark, which write mode is recommended for initial batch population versus subsequent incremental updates?
...
### Pregunta NaN: What metadata columns should ideally be stored alongside the chunk text in the Unity Catalog Delta table to support citations and hybrid filtering?
...
### Pregunta NaN: How does Auto Loader (`cloudFiles`) improve the document ingestion pipeline into Unity Catalog Delta tables?
...
### Pregunta NaN: When designing a customer service RAG application for a SaaS platform, which source documents provide the highest quality knowledge foundation?
...
### Pregunta NaN: What strategy ensures that an enterprise RAG assistant does not provide outdated answers when company policies change?
...
### Pregunta NaN: How does conducting a 'Gap Analysis' on user query logs help identify missing source documents for a RAG system?
...
### Pregunta NaN: When selecting knowledge sources for a financial compliance RAG agent, why is it critical to include both authoritative policy texts and structured regulatory tables?
...
### Pregunta NaN: If a company has both public documentation and highly confidential internal engineering wikis, how should source documents be organized in Unity Catalog?
...
### Pregunta NaN: What quality check should be performed on source PDF documents before ingesting them into an enterprise knowledge pipeline?
...
### Pregunta NaN: Why should source documents for a technical coding RAG assistant include runnable code examples alongside explanatory text?
...
### Pregunta NaN: Which retrieval evaluation metric measures the proportion of relevant documents retrieved out of all existing relevant documents in the corpus?
...
### Pregunta NaN: Which metric is most sensitive to the exact rank order of retrieved documents, heavily penalizing relevant documents that appear low in the list?
...
### Pregunta NaN: When evaluating a search engine where users only look at the first 3 results, which metric is most directly aligned with user satisfaction?
...
### Pregunta NaN: In Databricks Mosaic AI Agent Evaluation, which metric evaluates whether the retrieved chunks contain all the necessary information to answer the user question?
...
### Pregunta NaN: What is the primary role of a 'Golden Evaluation Dataset' in retrieval benchmarking?
...
### Pregunta NaN: If a retrieval system has High Precision@5 (0.90) but Low Recall@5 (0.30), what does this indicate about the retrieval behavior?
...
### Pregunta NaN: How does MLflow evaluate retrieval quality using an LLM-as-a-Judge approach?
...
### Pregunta NaN: What is Mean Average Precision (MAP) in the context of information retrieval?
...
### Pregunta NaN: What is the core concept behind **Parent-Document Retrieval** (or Small-to-Big retrieval)?
...
### Pregunta NaN: How does **Hierarchical Chunking** organize multi-level document repositories (e.g. Book -> Chapter -> Section -> Paragraph)?
...
### Pregunta NaN: What is **Contextual Chunking** (or Context-Enriched Chunking) as introduced by modern GenAI architectures?
...
### Pregunta NaN: In a **Multi-Vector Retriever** strategy, what is indexed in the vector store versus what is stored in the document store?
...
### Pregunta NaN: What is **Hypothetical Document Embeddings (HyDE)** and how does it alter retrieval?
...
### Pregunta NaN: When implementing advanced table chunking, why is converting HTML / Markdown tables into individual row-level summary sentences effective?
...
### Pregunta NaN: What is the primary benefit of **Sentence Window Retrieval**?
...
### Pregunta NaN: What is the primary role of a **Re-ranker** (Cross-Encoder) in a two-stage retrieval pipeline?
...
### Pregunta NaN: Why is a two-stage retrieval architecture (Bi-Encoder Vector Search + Cross-Encoder Re-ranker) preferred over using a Cross-Encoder for the entire corpus?
...
### Pregunta NaN: Which metric is most suitable for evaluating the quality improvement delivered by adding a re-ranking stage to a retrieval pipeline?
...
### Pregunta NaN: In a hybrid search system combining BM25 keyword search and Dense Vector Search, how does **Reciprocal Rank Fusion (RRF)** function?
...
### Pregunta NaN: What is a potential trade-off when adding a cross-encoder re-ranking step to an interactive real-time RAG application?
...
### Pregunta NaN: How does a re-ranker help reduce hallucinations in the downstream LLM generator?
...
### Pregunta NaN: When configuring a two-stage retrieval pipeline, what is a typical rule of thumb for candidate retrieval count (k1) and final re-ranked count (k2)?
...
### Pregunta NaN: When subclassing `mlflow.pyfunc.PythonModel` to build a custom GenAI serving pipeline, which method must be overridden to define inference logic?
...
### Pregunta NaN: In a custom MLflow PyFunc model, how should static configuration artifacts (like prompt templates, tokenizers, or index client configs) be loaded?
...
### Pregunta NaN: How does pre-processing in a PyFunc model help protect downstream Foundation Model APIs?
...
### Pregunta NaN: What is the role of post-processing inside a PyFunc model's `predict()` method?
...
### Pregunta NaN: How can custom Python dependencies be specified when logging a custom PyFunc model in MLflow?
...
### Pregunta NaN: When a PyFunc model needs to call a Databricks Foundation Model Serving endpoint internally, which client library should be used inside `predict()`?
...
### Pregunta NaN: What data format does a PyFunc `predict()` method accept by default according to standard MLflow Model Signatures?
...
### Pregunta NaN: How does Databricks Model Serving authenticate and access dependent Unity Catalog resources (like Vector Search indexes and Delta tables) securely?
...
### Pregunta NaN: Which Unity Catalog privilege is required for a Model Serving endpoint to query a Vector Search index in catalog `finance` and schema `rag`?
...
### Pregunta NaN: What is the recommended method for granting an external application access to a Databricks Model Serving endpoint?
...
### Pregunta NaN: How do IP Access Lists protect Databricks Model Serving endpoints?
...
### Pregunta NaN: What role do Unity Catalog Row-Level Security (RLS) and Column Masking play when a RAG agent queries Delta tables?
...
### Pregunta NaN: If a developer needs to store external API keys (e.g. for external Slack or CRM integrations) used by an agent, where should they be stored in Databricks?
...
### Pregunta NaN: What is the correct LCEL syntax to compose a standard prompt-model-output pipeline in LangChain?
...
### Pregunta NaN: How do you invoke an LCEL chain synchronously with a dictionary of input variables?
...
### Pregunta NaN: Which method enables asynchronous streaming of tokens as they are generated by an LCEL chain?
...
### Pregunta NaN: How do you run batch inference over a list of 100 questions simultaneously using an LCEL chain?
...
### Pregunta NaN: What happens if a required variable defined in a `PromptTemplate` (e.g. `{context}`) is missing from the input dictionary passed to `chain.invoke()`?
...
### Pregunta NaN: How do you attach custom runtime parameters (such as `temperature` or `max_tokens`) to an LLM inside an LCEL chain dynamically?
...
### Pregunta NaN: Which essential components must be defined and packaged when building a complete RAG application in Databricks MLflow?
...
### Pregunta NaN: What is the purpose of logging an **Input Example** alongside a model in MLflow (`mlflow.langchain.log_model(..., input_example=...)`)?
...
### Pregunta NaN: Why is defining an explicit **Model Signature** critical for enterprise Model Serving endpoints?
...
### Pregunta NaN: When deploying a RAG chain to Model Serving, where should the Vector Search endpoint name and index name be configured?
...
### Pregunta NaN: Which MLflow model flavor is best suited for deploying complex RAG systems that require custom Python logic alongside LangChain and Vector Search?
...
### Pregunta NaN: Why should `pip_requirements` include pinned version numbers (e.g. `langchain==0.2.14`, `databricks-vectorsearch==0.40`) when logging a RAG model?
...
### Pregunta NaN: What role does the **Retriever** component play in a production RAG application?
...
### Pregunta NaN: How do you register an MLflow model into Unity Catalog using the 3-level namespace convention?
...
### Pregunta NaN: Which Unity Catalog privilege is required for a data scientist to register a new model in schema `prod.genai`?
...
### Pregunta NaN: What is the recommended approach for promoting a model across lifecycle stages (e.g. Dev -> Staging -> Prod) in Unity Catalog?
...
### Pregunta NaN: How does registering models in Unity Catalog improve enterprise security compared to legacy workspace model registries?
...
### Pregunta NaN: When an MLflow model is registered in Unity Catalog, what artifact information is automatically captured and tracked in the UI?
...
### Pregunta NaN: How do you load a specific model version with alias `@Champion` from Unity Catalog in Python?
...
### Pregunta NaN: Which two main types of Vector Search indexes exist in Databricks Mosaic AI Vector Search?
...
### Pregunta NaN: What is the difference between a **Delta Sync Index (Triggered)** and a **Delta Sync Index (Continuous)** in Mosaic AI Vector Search?
...
### Pregunta NaN: In a **Managed Embeddings** Vector Search Index, who computes the embedding vectors for incoming text chunks?
...
### Pregunta NaN: How do you query a Mosaic AI Vector Search index using the Python SDK?
...
### Pregunta NaN: What role do **Filters** (e.g. `filters={'category': 'legal', 'year': 2025}`) play during a Vector Search query?
...
### Pregunta NaN: What compute resource is required before you can create a Mosaic AI Vector Search index in Databricks?
...
### Pregunta NaN: How do you trigger a manual synchronization of a Triggered Delta Sync Index in Python?
...
### Pregunta NaN: What are **Databricks Foundation Model APIs** and what deployment modes do they offer?
...
### Pregunta NaN: How can an external application call a Databricks Foundation Model Serving endpoint using the standard OpenAI Python client library?
...
### Pregunta NaN: What is the benefit of **External Models** support in Databricks Model Serving (e.g. proxying to Azure OpenAI, Anthropic Claude, or AWS Bedrock)?
...
### Pregunta NaN: What automatic scaling behavior does Databricks Model Serving provide for custom PyFunc and Foundation Model endpoints?
...
### Pregunta NaN: How does **AI Gateway Rate Limiting** protect enterprise Model Serving endpoints from runaway costs or denial-of-service traffic?
...
### Pregunta NaN: When deploying a Python model to Model Serving, what container environment does Databricks construct automatically?
...
### Pregunta NaN: What approximate nearest neighbor (ANN) indexing algorithm is utilized under the hood in Mosaic AI Vector Search for ultra-fast similarity search?
...
### Pregunta NaN: What is the primary advantage of **Mosaic AI Vector Search** being natively integrated with Unity Catalog?
...
### Pregunta NaN: In Mosaic AI Vector Search, how are deleted rows in the source Delta table handled by a Delta Sync Index?
...
### Pregunta NaN: What distance metrics are supported when creating a Mosaic AI Vector Search index?
...
### Pregunta NaN: What happens when a Vector Search index is queried with metadata filters on columns that are indexed alongside the vectors?
...
### Pregunta NaN: Why is an internal sync pipeline created when you set up a Delta Sync Index in Mosaic AI Vector Search?
...
### Pregunta NaN: What is the SQL function `ai_query()` in Databricks and what is its primary use case?
...
### Pregunta NaN: What is the correct SQL syntax to classify customer reviews in table `reviews` using `ai_query()` on Databricks?
...
### Pregunta NaN: When running large-scale batch inference over 100 million customer records using `ai_query()`, what compute resource should be used for optimal throughput and scaling?
...
### Pregunta NaN: How does `ai_query()` handle structured JSON output return types in Databricks SQL?
...
### Pregunta NaN: In batch scoring pipelines, why is using `ai_query()` in SQL often preferred over running custom Python loops with `requests.post()` in a notebook?
...
### Pregunta NaN: How can `ai_query()` be used to generate vector embeddings in batch over a Delta table?
...
### Pregunta NaN: A company has a 50-million vector document corpus updated once every 24 hours at midnight. Which Vector Search configuration optimizes cost while fulfilling requirements?
...
### Pregunta NaN: A mission-critical financial news search tool requires that breaking news articles be searchable within 5 seconds of publication. What configuration is required?
...
### Pregunta NaN: What factor dictates the memory and compute capacity needed for a Vector Search Endpoint?
...
### Pregunta NaN: When configuring a Vector Search index, how does embedding dimension selection (e.g. 384 vs 1536) impact cost?
...
### Pregunta NaN: If a Vector Search endpoint experiences high P99 query latency during peak hours, which adjustment resolves the bottleneck?
...
### Pregunta NaN: In a Direct Vector Access Index, what is the developer responsible for managing that Delta Sync handles automatically?
...
### Pregunta NaN: Why is a persistent datastore (such as a Delta Lake table or Lakehouse Key-Value store) required for production multi-turn agent systems?
...
### Pregunta NaN: When designing a conversation memory table in Delta Lake, what schema columns are essential?
...
### Pregunta NaN: How can LangChain's `RunnableWithMessageHistory` be configured to use a persistent Delta Lake memory store in Databricks?
...
### Pregunta NaN: What strategy prevents persistent conversation memory from exceeding an LLM's context window over long-running 100-turn chat sessions?
...
### Pregunta NaN: For low-latency key-value state retrieval (e.g. sub-5ms user profile lookup during agent execution), which storage architecture is recommended alongside the Lakehouse?
...
### Pregunta NaN: How does Unity Catalog govern persistent conversation memory tables containing customer chat logs?
...
### Pregunta NaN: In a production CI/CD pipeline for a GenAI application on Databricks, how should prompt templates and agent configurations be version controlled?
...
### Pregunta NaN: What automated testing step should run in a CI/CD pull request before promoting a new RAG agent version to staging?
...
### Pregunta NaN: When deploying a Vector Search index update in a CI/CD pipeline, how is zero-downtime search maintained?
...
### Pregunta NaN: How do **Databricks Asset Bundles (DABs)** facilitate CI/CD for Generative AI applications?
...
### Pregunta NaN: What is a **Canary Deployment** for a Model Serving endpoint in Databricks?
...
### Pregunta NaN: How should individual tools used by an agent (e.g. UC Functions, calculator, database lookups) be tested in a unit-testing pipeline?
...
### Pregunta NaN: What is the **Model Context Protocol (MCP)** and what problem does it solve in agentic AI architectures?
...
### Pregunta NaN: When an agent integrates a **Custom MCP Server** in Databricks, how does the agent discover available tools exposed by that server?
...
### Pregunta NaN: What is the security model when an agent invokes a tool hosted on an external managed MCP server?
...
### Pregunta NaN: In what scenario is an MCP server preferred over hardcoding custom API integration scripts inside an agent notebook?
...
### Pregunta NaN: How does an agent handle an error returned by an MCP tool execution (e.g. `isError: true` in tool result)?
...
### Pregunta NaN: What is **Prompt Version Control** and why is it essential in production GenAI engineering?
...
### Pregunta NaN: How does the **MLflow Prompt Registry / Prompt Management** feature support prompt lifecycles in Databricks?
...
### Pregunta NaN: When a prompt update improves accuracy on English queries but inadvertently breaks German queries, how does prompt version control resolve the incident?
...
### Pregunta NaN: What role do Prompt Metadata tags (e.g. `author`, `target_model`, `temperature`, `use_case`) play in enterprise prompt governance?
...
### Pregunta NaN: How should prompt template changes be validated before promoting from `@Staging` to `@Champion`?
...
### Pregunta NaN: What is **Databricks Apps** and how does it enable deploying custom web UIs for GenAI agents?
...
### Pregunta NaN: When integrating a Databricks GenAI agent into corporate collaboration platforms (such as Slack or Microsoft Teams), what architecture is standard?
...
### Pregunta NaN: Why is **Streaming Token Response (SSE / WebSockets)** critical in interactive chat interfaces (Streamlit / React)?
...
### Pregunta NaN: What interactive UI element should be included in a production agent interface to collect real-time human feedback for continuous improvement?
...
### Pregunta NaN: How does **Databricks Review App** authenticate business stakeholders testing an agent prototype?
...
### Pregunta NaN: What is the primary limitation of traditional n-gram matching metrics like **BLEU** and **ROUGE** when evaluating modern Generative AI responses?
...
### Pregunta NaN: How does **BERTScore** improve over BLEU/ROUGE for evaluating generated text against reference text?
...
### Pregunta NaN: Why is **Perplexity (PPL)** a problematic metric for evaluating factual accuracy in customer-facing RAG chatbots?
...
### Pregunta NaN: What traditional metric evaluates string distance based on the minimum number of single-character edits (insertions, deletions, substitutions)?
...
### Pregunta NaN: In an extractive Question-Answering benchmark, how is **Exact Match (EM)** metric defined?
...
### Pregunta NaN: Why has the industry widely shifted from traditional n-gram metrics to **LLM-as-a-Judge** for evaluating complex reasoning and open-ended generation?
...
### Pregunta NaN: What specific insights does **MLflow Tracing** provide when diagnosing a multi-agent system failure?
...
### Pregunta NaN: How do you enable automated tracing for LangChain applications in MLflow with a single line of code?
...
### Pregunta NaN: When an agent enters an infinite loop repeatedly calling the same tool without answering, how does tracing help engineers resolve the bug?
...
### Pregunta NaN: How can custom Python functions within a GenAI pipeline be instrumented as spans in an MLflow trace?
...
### Pregunta NaN: When investigating a sudden latency spike from 500ms to 8,000ms in a RAG pipeline, which trace component pinpoints the culprit?
...
### Pregunta NaN: Where are MLflow Traces stored and accessed when running agents inside Databricks?
...
### Pregunta NaN: Which key operational metrics must be monitored continuously to manage the **Cost** of a production GenAI application?
...
### Pregunta NaN: What are the two primary **Latency** metrics monitored in interactive streaming LLM applications?
...
### Pregunta NaN: In Mosaic AI Agent Evaluation, which triad of metrics comprehensively assesses **RAG Quality**?
...
### Pregunta NaN: What metric tracks the percentage of production requests that receive HTTP 429 (Too Many Requests) or HTTP 500 (Internal Server Error) status codes?
...
### Pregunta NaN: How does tracking **Token Cache Hit Rate** (e.g. Prompt Caching) help optimize enterprise LLM applications?
...
### Pregunta NaN: What is **P99 Latency** and why is it critical for monitoring enterprise GenAI applications?
...
### Pregunta NaN: What is the **LLM-as-a-Judge** evaluation methodology in Databricks Mosaic AI?
...
### Pregunta NaN: What is **Position Bias** in LLM-as-a-Judge pairwise evaluation (comparing Model A vs Model B) and how is it mitigated?
...
### Pregunta NaN: What is **Verbosity Bias** in LLM judges and how is it controlled?
...
### Pregunta NaN: How does **Self-Enhancement Bias (Self-Preference Bias)** affect LLM judges?
...
### Pregunta NaN: What are the essential components of a well-engineered **LLM Judge Evaluation Prompt / Rubric**?
...
### Pregunta NaN: In Databricks Mosaic AI Agent Evaluation, how does the system provide transparent visibility into LLM-as-a-Judge scores?
...
### Pregunta NaN: When designing a unified metric logging architecture for diverse stakeholders (Engineers, Product Managers, Compliance Officers), what data should be captured in the central Delta table?
...
### Pregunta NaN: How do **Databricks Lakehouse Dashboards (AI/BI Dashboards)** leverage logged evaluation tables to serve executive stakeholders?
...
### Pregunta NaN: What privacy safeguard must be applied before logging raw production user prompt inputs and completions into shared Lakehouse metric tables?
...
### Pregunta NaN: How should automated alerts be configured on GenAI metric tables to notify engineering teams of critical incidents?
...
### Pregunta NaN: Why is it important to log the **Prompt Template Version ID** and **Model Serving Endpoint Version** alongside every production request record?
...
### Pregunta NaN: In a multi-tenant enterprise GenAI deployment, which metadata field is critical for logging to enable cost chargebacks across departments?
