...
[
  ...
  },
  {
    ...
    "options": [
      {
        "id": "a",
        "text": "Testing the latency of real-time streaming workloads"
      },
      {
        "id": "b",
        "text": "Performing analysis-specific ETL on gold-layer tables"
      },
      {
        "id": "c",
        "text": "Predicting customer churn by automatically developing time-series models"
      },
      {
        "id": "d",
        "text": "Monitoring the performance of deployed machine learning solutions"
      }
    ],
    "correctIds": [
      "b"
    ],
    "explanation": "Databricks SQL is designed for analytics users to query and refine business-ready data (gold layer) for specific reports or dashboards, effectively performing last-mile ETL.",
    "domain": "Dashboards & Visualizations"
  },
  {
    "id": "db-da-75",
...
,
{
    ...
    },
    {
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "Despite having SELECT permission on tables and USAGE on schemas, a data analyst is unable to access the objects.\n\nWhich action enables the data analyst to access the objects?",
        "options": [
            {
                "id": "a",
                "text": "Grant OWNERSHIP of the table to the Data Analyst."
        ...
        "explanation": "In Unity Catalog the catalog is the top-level container for schemas and tables. Even when a user has SELECT on a table and USAGE on a schema, they cannot reference the table unless the catalog itself is usable by that user. The required privilege to make a catalog visible is USE CATALOG; granting this allows the analyst to address objects within it without altering ownership or granting broader administrative rights. Option A (OWNERSHIP) is overly broad and reserved for administrators. Option B (MANAGE DATABASE) is an admin privilege for creating/altering/dropping databases. Option C (MANAGE TABLE) controls table-level administration, not read access.",
        "domain": "Securing Data"
    },
    {
        "id": "db-da-196",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst is reviewing the status of alerts in the Databricks SQL Alerts listing page. One alert was recently executed, and the monitored value met the defined condition and exceeded the threshold, meaning the alert criteria were met.\n\nWhich status should appear for this alert in the listing?",
        "options": [
            {
                "id": "a",
                "text": "Status: YES"
        ...
        "explanation": "When an alert's condition evaluates to true and the monitored metric exceeds the configured threshold, Databricks marks the alert as TRIGGERED. This status explicitly indicates that the alert has been activated due to the condition being satisfied. YES is not a recognized alert status in the Databricks SQL Alerts UI. OK denotes that the condition is not met (i.e., the alert is healthy). SUCCESS refers to the successful execution of the underlying query or job, not to the logical evaluation of the alert condition.",
        "domain": "Dashboards & Visualizations"
    },
    {
        "id": "db-da-197",
        ...
        "explanation": "Time-travel querying in Delta Lake lets you query the table at any retained snapshot using a timestamp, version number, or offset (VERSION AS OF, TIMESTAMP AS OF, etc.). This provides a read-only view of the table's state at the desired point in time without altering the current data. RESTORE (A) overwrites or creates a new table — unnecessary for comparison. VACUUM (B) removes old files and actually reduces the time-travel window. CLONE with VERSION AS OF (D) creates a separate copy; time travel directly on the original table is more idiomatic for a simple comparison.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-198",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst wants to generate insights from large, complex datasets. The analyst needs to quickly understand the meaning of various data columns, ask questions in natural language, and receive AI-driven recommendations for optimizing data queries and workflows.\n\nWhich Databricks component is primarily responsible for enabling these capabilities?",
        "options": [
            {
                "id": "a",
                "text": "Data Intelligence Engine"
        ...
        "explanation": "The Data Intelligence Engine (DIE) is Databricks' built-in engine that catalogs data assets, infers schemas and semantics, and surfaces natural-language search across the lakehouse. It enables analysts to discover column-level meaning, receive query-optimization suggestions, and get workflow recommendations. Unity Catalog (B) provides centralized governance and access control, not AI-driven discovery. Genie Spaces (C) focuses on ML workflow orchestration. Databricks Assistant (D) is a conversational UI layer rather than the underlying engine that powers catalog-level semantic understanding.",
        "domain": "Platform Understanding"
    },
    {
        "id": "db-da-199",
    ...
    },
    {
        ...
        "explanation": "In Data Vault 2.0, Hubs store unique business keys, Satellites store descriptive attributes, and Links model relationships between those keys. When multiple hubs are involved in a relationship — for example, a sales transaction that links a customer hub and a product hub — the relationship is captured by a Link Table that holds foreign-key references to each participating hub. Hub Table (A) only stores business keys. Satellite Table (B) captures attribute data, not structural relationships. Reference Table (D) is not a defined component in the Data Vault methodology for modeling relationships.",
        "domain": "Data Modeling"
    },
    {
        "id": "db-da-204",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst is using Databricks Unity Catalog. The datasets are tagged by sensitivity, and confidential data is marked with the tag key confidential. The data analyst needs to quickly find all tables tagged as confidential to review their access permissions in the Databricks workspace search bar.\n\nWhich search key text should the data analyst use to find these tables?",
        "options": [
            {
                "id": "a",
                "text": "confidential:true"
        ...
        "explanation": "Unity Catalog allows tags to be queried directly in the workspace search bar using the tag: prefix followed by the tag key name. The correct syntax is tag:confidential, which returns all objects (tables, views, etc.) that have a tag with the key confidential, regardless of the tag's value. Option A (confidential:true) is not a valid search pattern. Option B (tag=confidential) uses an equals sign, which Unity Catalog does not accept in search queries. Option D includes extra wording and spaces that are not part of the supported search syntax.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-205",
        ...
        "explanation": "The Gold layer holds denormalized, curated models that are purpose-built for analytics, reporting, and machine-learning workloads. Gold tables are the final, production-ready datasets that have passed through cleansing, enrichment, and standardization in Bronze and Silver. They are typically optimized for query performance (partitioning, Z-ordering, clustering). Option A mischaracterizes Gold — backup/recovery is a cross-layer concern. Option B describes the Bronze layer (raw ingestion). Option C aligns more with the Silver layer, where data is cleaned and deduplicated.",
        "domain": "Data Modeling"
    },
    {
        "id": "db-da-206",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst team is building reliable, production-grade data pipelines on Databricks. They require a solution that allows them to define data transformations declaratively, automatically enforce data quality expectations, and handle errors without manual intervention.\n\nWhich Databricks capability addresses these requirements?",
        "options": [
            {
                "id": "a",
                "text": "Lakeflow Tables"
        ...
        "explanation": "Lakeflow Spark Declarative Pipelines provide a declarative, YAML/JSON-based pipeline definition where transformations are specified as logical steps rather than hand-coded Spark jobs, enabling true pipeline-as-code. They include built-in data-quality enforcement (schema validation, null-checks, custom quality rules) applied automatically at each stage, and automatic error handling with defined fallback actions. Lakeflow Tables (A) focuses on creating and managing Delta tables but does not provide a declarative pipeline engine. Lakeflow Jobs (C) handles orchestration and scheduling of existing jobs without built-in data-quality checks. Lakeflow Connect (D) offers low-code connectors for ingesting data from external sources, not transformation definition.",
        "domain": "Platform Understanding"
    },
    {
        "id": "db-da-207",
    ...
    },
    {
        ...
        "explanation": "Dynamic Views can expose only the non-PII columns to users who are not members of a privileged role, while the underlying table still holds the full PII. The IS_MEMBER() function evaluates the caller's role at query time, ensuring that sensitive fields (e.g., email, birthdate) are never returned to unauthorized users. Option A (open SELECT) defeats least-privilege access. Option C (shared CSV with folder ACLs) is coarse-grained and does not provide column-level masking. Option D (permanently masking in the source table) corrupts downstream analytics and complicates data lineage.",
        "domain": "Securing Data"
    },
    {
        "id": "db-da-209",
    ...
    },
    {
        ...
        "explanation": "In Unity Catalog, privileges are inherited down the object hierarchy. When a privilege is granted at a higher level (like a catalog or schema), it is automatically propagated to all child objects contained within it. For example, a privilege granted on a schema is inherited by all tables and views in that schema. This reduces administrative overhead. If a more specific permission is granted at a lower level, it can override or complement the inherited privilege.",
        "domain": "Securing Data"
    },
    {
        "id": "db-da-211",
    ...
    },
    {
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst needs to improve the performance of a daily sales report that aggregates data from a large transactions table. The report is run multiple times a day, but the underlying data only changes once daily.\n\nWhich approach should the analyst use to improve performance while minimizing redundant computation?",
        "options": [
            {
                "id": "a",
                "text": "Use a standard view and schedule a notebook to cache results."
        ...
        "explanation": "Materialized views precompute the result of a query and store it on disk. They are ideal for cases where the source data changes infrequently (like once a day) compared to how often it is queried. This avoids redundant computations. Standard and dynamic views recalculate results on every run, and streaming tables are for continuous data flows, not for one-time daily aggregations.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-216",
    ...
    },
    {
        ...
        "explanation": "To return only rows where multiple keys match between tables, an INNER JOIN with multiple join conditions in the ON clause using AND is the correct method. Option A only checks one key. Option B is a shorthand that only works with identical column names. Option D is a LEFT JOIN, which returns non-matching rows from the left table.",
        "domain": "Executing Queries"
    },
    {
        "id": "db-da-218",
    ...
    },
    {
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst needs to programmatically mark the sales_data table in Unity Catalog as deprecated so that users across their organization can clearly see that it is no longer recommended for use.\n\nWhich SQL command should they use?",
        "options": [
            {
                "id": "a",
                "text": "ALTER TABLE sales_data SET TAGS ('deprecated' = 'true');"
            },
            {
                "id": "b",
                "text": "UPDATE TABLE sales_data ADD COMMENT 'Deprecated dataset';"
            },
            {
                "id": "c",
                "text": "ALTER TABLE sales_data SET TBLPROPERTIES ('system.certification_status'='deprecated');"
            },
            {
                "id": "d",
                "text": "ALTER TABLE sales_data SET TAGS ('system.certification_status' = 'deprecated');"
            }
        ],
        "correctIds": [
            "d"
        ],
        "explanation": "Unity Catalog uses system-controlled tags ('system.certification_status') to officially mark datasets as 'certified' or 'deprecated'. Using the 'SET TAGS' syntax ensures the status is recognized by Databricks tools and displayed correctly in the Catalog Explorer UI. Option A is a custom tag but lacks system visibility. Option B is invalid syntax. Option C uses Hive-specific table properties instead of UC tags.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-220",
    ...
    },
    {
        ...
        "explanation": "When filtering based on aggregate results (COUNT, AVG), the conditions must be placed in the HAVING clause, which is evaluated after grouping. The WHERE clause is for row-level filtering and cannot reference aggregate functions. Option A correctly uses HAVING after GROUP BY to apply both conditions.",
        "domain": "Executing Queries"
    },
    {
        "id": "db-da-222",
        ...
        "explanation": "Unity Catalog handles structured data through standard tables and unstructured data through 'Volumes'. Volumes represent directories of files (PDFs, images, CSVs, etc.) stored in external cloud locations but managed under the same governance and permissions model as tables. Option B is incorrect because no conversion is needed. Option C is too restrictive. Option D is partially correct about blobs but ignores the core mechanism of 'Volumes'.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-223",
    ...
    },
    {
        ...
        "options": [
            ...
            {
                "id": "c",
                "text": "SQL Warehouses are general-purpose compute resources that support Python, Scala, R, and SQL workloads, with automatic library management."
            },
            {
                "id": "d",
                "text": "SQL Warehouses function as data storage systems that maintain cached query results and provide direct access to Delta tables without requiring additional compute resources."
        ...
        "explanation": "SQL Warehouses are purpose-built for SQL workloads and BI connectivity. They utilize the Photon engine for high-performance vectorized execution and are optimized for low-latency concurrent queries from BI tools like Tableau or PowerBI. Unlike all-purpose clusters (C), they are SQL-specific and do not run arbitrary Python/Scala code.",
        "domain": "Platform Understanding"
    },
    {
        "id": "db-da-226",
        ...
        "explanation": "The 'Configure > Context' area of a Genie space provides a dedicated UI for binding sample questions to validated SQL queries. This approach ensures version-controlled guidance that Genie uses to map natural language queries more accurately. Option B (Description field) is for high-level metadata only, and Options A and C add unnecessary external dependencies.",
        "domain": "AI/BI Genie Spaces"
    },
    {
        "id": "db-da-227",
        ...
        "explanation": "Query Insights is specifically designed to surface per-query metrics (duration, rows read, bytes scanned) over its execution history. This allows analysts to correlate slow-downs with specific runs and pinpoint bottlenecks. While Query Profiler (C) provides deep details on a *single* run, it lacks the aggregated historical 'insight' view for performance over time.",
        "domain": "Analyzing Queries"
    },
    {
        "id": "db-da-228",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A BI analyst has created a Genie space containing customer sales data and needs to share it with external stakeholders who are not workspace members. The stakeholders require the ability to ask questions about the data but should not have access to the underlying Databricks workspace or compute resources. How should the BI analyst share the Genie space with external stakeholders?",
        "options": [
            {
                "id": "a",
                "text": "Share the Genie space with embedded credentials and assign the stakeholders as account users with published dashboard access."
        ...
        "explanation": "By publishing a Genie space (or dashboard) with embedded credentials, external stakeholders added as 'account users' can interact with the natural language interface without having direct access to the workspace, clusters, or raw table explorer. This ensures isolation and security while fulfilling the requirement for interaction. Options C and D violate the requirement to keep them out of the underlying workspace.",
        "domain": "AI/BI Genie Spaces"
    },
    {
        "id": "db-da-229",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A company uses a star schema to analyze sales data. The sales fact table records transactions, and dimension tables include products, stores, and dates. How are primary and foreign keys used to enable efficient joining between these tables?",
        "options": [
            {
                "id": "a",
                "text": "The fact table and dimension tables are joined using arbitrary columns, and primary/foreign key relationships are not required in a star schema."
        ...
        "explanation": "In a star schema, dimension tables contain a primary key (often a surrogate key) that uniquely identifies a row. The central fact table contains foreign keys that point to these dimension primary keys, creating a one-to-many relationship. This structure is fundamental for data integrity and optimized join performance in analytical environments.",
        "domain": "Platform Understanding"
    },
    {
        "id": "db-da-230",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A BI analyst is designing a Databricks SQL dashboard that tracks monthly revenue for three product categories across two years. Leadership wants a chart that makes it straightforward to compare each category's trend over time and quickly spot seasonal patterns or declines without relying on additional filters or calculations. Which visualization type in Databricks SQL will clearly communicate the revenue trends and category comparisons described?",
        "options": [
            {
                "id": "a",
                "text": "Multi-series line chart plotting month on the x-axis and revenue on the y-axis for each category."
        ...
        "explanation": "A multi-series line chart is ideal for showing continuous trends over time while allowing direct side-by-side comparison of multiple categories. It makes seasonality and growth/decline patterns instantly visible across the entire timeline.",
        "domain": "Dashboards & Visualizations"
    },
    {
        "id": "db-da-231",
    ...
    },
    {
        ...
        "explanation": "The killer feature of Liquid Clustering is its flexibility. Unlike traditional Hive-style partitioning, which physically separates data into directories based on columns (requiring a full rewrite of the table to change the partition scheme), Liquid Clustering stores clustering keys in metadata. This allows an analyst to change the clustering key on-the-fly without a costly data movement operation.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-234",
        ...
        "lang": "en",
        "type": "multiple_choice",
        "prompt": "A data analyst is configuring a Genie space to support natural language questions about product sales, including fields like region_code, sku, and channel_type. The analyst notices that Genie often generates ambiguous or incorrect queries due to unclear column meanings.\n\nWhich two actions will help to improve Genie's accuracy and relevance when interpreting user questions? (Choose two.)",
        "options": [
            {
                "id": "a",
                "text": "Provide sample values for ambiguous columns to help Genie infer the data's meaning and usage."
        ...
        "explanation": "To reduce ambiguity in LLM-driven query generation, providing semantic context is key. Adding column descriptions in Unity Catalog (D) gives direct metadata context for sku and region_code. Writing domain-specific instructions (B) helps the model understand complex business logic (e.g., how 'market' maps to 'region_code'). Example queries (E) are useful but less scalable/robust than structured metadata and instructions.",
        "domain": "AI/BI Genie Spaces"
    },
    {
        "id": "db-da-235",
        ...
        "explanation": "Databricks SQL dashboards can be 'published' to create a public/external URL. Using embedded credentials (service principal) allows anyone with the link to view the live dashboard and interact with filters without needing a Databricks account/workspace login. Option D is incorrect because PDF is static and lacks interactivity.",
        "domain": "Dashboards & Visualizations"
    },
    {
        "id": "db-da-236",
        ...
        "options": [
            ...
            {
                "id": "c",
                "text": "Well-annotated datasets registered to Unity Catalog, curated sample queries, and domain-specific instructions to guide natural language translation."
            },
            {
                "id": "d",
                "text": "A dedicated physical server allocated for the Genie space to maximize availability and scalability."
        ...
        "explanation": "An effective Genie space relies on 'high-fidelity' metadata. This is achieved by having datasets strictly governed in Unity Catalog (with annotations), providing sample queries for few-shot learning, and including specific instructions to steer the natural language to SQL translator. Options A, B, and D describe unnecessary architectural complexity.",
        "domain": "AI/BI Genie Spaces"
    },
    {
        "id": "db-da-237",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A company has started using the Databricks Platform and currently has only one workspace. They need to manage data access across different teams and want to ensure proper isolation and security controls. Given this setup, which approach should be used?",
        "options": [
            {
                "id": "a",
                "text": "Create a unique catalog with all schemas inside of it."
        ...
        "explanation": "Catalogs act as independent namespaces in Unity Catalog, allowing you to assign distinct permission policies, retention settings, and access-control rules to each environment. Provisioning separate catalogs for pro, dev, and test is the recommended pattern because it provides explicit, enforceable isolation, clear ownership boundaries, and granular access control.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-238",
    ...
    },
    {
        ...
        "lang": "en",
        "type": "multiple_choice",
        "prompt": "A data analyst is migrating data lakehouse workloads to Databricks. The analyst manages a large table with thousands of columns and several billion rows. The analyst is concerned about scan performance, query speed, and handling the overhead of processing such a large volume of data.\n\nIn which two ways does the Photon engine help the team address these challenges? (Choose two.)",
        "options": [
            {
                "id": "a",
                "text": "Photon uses a caching layer to transcode data into a CPU-efficient format for faster scan performance."
        ...
        "explanation": "Photon addresses big data challenges through two core mechanisms: (1) Caching & Transcoding (A) - It stores data in a CPU-friendly, in-memory layout that avoids costly format conversions and speeds up scans on wide tables; (2) Columnar Vectorized Execution (E) - It processes data in large batches (vectors) rather than row-by-row, which drastically reduces CPU overhead for scans and aggregates on billions of rows. Row-by-row processing (B) is slower, multi-cluster parallelism (C) is a cluster-level concern, and file merging (D) is a storage-layer concern (e.g., Delta Optimize).",
        "domain": "Executing Queries"
    },
    {
        "id": "db-da-242",
        ...
        "explanation": "Databricks Marketplace is directly integrated with Unity Catalog and allows consumers to access live datasets sharing via Delta Sharing. This eliminates the need for data replication or ETL, as the data is queried in-place. All access is governed by Unity Catalog permissions, fulfilling requirements for fast and secure third-party data enrichment.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-243",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst uses Databricks to create a dashboard that monitors daily sales volumes and store performance. To ensure that area managers always see up-to-date data without having to manually intervene, the data analyst wants to automate the dashboard data refresh.\n\nWhich process should the data analyst use to automatically refresh the data displayed, avoiding manual intervention?",
        "options": [
            {
                "id": "a",
                "text": "Set the Dashboard query to 'auto-update' by using a browser extension that reloads the tab at set intervals."
        ...
        "explanation": "Databricks dashboards support built-in scheduling for data refreshes. This allows analysts to configure a cadence (every few minutes, hourly, daily, etc.) that automatically executes the underlying queries without manual intervention from any user. This approach leverages Databricks' native job-scheduling capabilities, ensuring that all stakeholders see the most current metrics upon opening the dashboard. Other options like browser extensions (A), manual PDF exports (B), or manual refresh requests (D) do not provide scalable, automated server-side data freshness.",
        "domain": "Dashboards & Visualizations"
    },
    {
        "id": "db-da-244",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst needs to automate scheduled ETL jobs that ingest and transform data from multiple sources. The analyst wants to use multiple programming languages for flexibility and ensure that the pipeline can automatically handle schema changes as new data arrives.\n\nWhich approach in Databricks should be used to meet this requirement?",
        "options": [
            {
                "id": "a",
                "text": "Databricks Workflows with Apache Spark notebooks and manual schema management"
        ...
        "explanation": "Lakeflow Spark Declarative Pipelines (formerly known as Delta Live Tables or DLT) are purpose-built to orchestrate multi-source ETL. They support multiple languages (SQL, Python), provide built-in scheduling, and—when combined with Auto Loader—automatically detect and adapt to evolving schemas (schema evolution/drift) without manual intervention. Manual schema management (A), fixed schemas (C), or standard cron/orchestration (B) lack the automated schema-handling and integrated multi-language declarative framework required for this use case.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-245",
    ...
    },
    {
        ...
        "type": "single_choice",
        "prompt": {
            "text": "A data analyst is given a sales table with the columns product_id, sale_date, and sale_amount. The analyst's goal is to calculate the total sales per product per day, with exactly one row per product per day. They first try the following SQL query:\n\nSELECT product_id, sale_date, sale_amount, SUM(sale_amount) OVER (PARTITION BY product_id ORDER BY sale_date) AS daily_total FROM sales ORDER BY product_id, sale_date;\n\nHowever, this produces multiple rows for the same product and date instead of one aggregated row per product/date combination. Which corrected SQL query will produce exactly one row per product per day with the total sales for that day?",
            "isCode": false
        },
        "options": [
            {
            ...
            {
                "id": "b",
                "text": "SELECT product_id, sale_date, SUM(sale_amount) AS daily_total FROM sales GROUP BY product_id, sale_date ORDER BY product_id, sale_date",
                "isCode": true
            },
            {
                "id": "c",
            ...
            {
                "id": "d",
                "text": "SELECT product_id, sale_date, sale_amount, SUM(sale_amount) OVER (PARTITION BY product_id ORDER BY sale_date) AS daily_total FROM sales",
                "isCode": true
            }
        ],
        "correctIds": [
        ...
        "explanation": "In SQL, the GROUP BY clause is specifically designed to collapse multiple rows that share the same values in specified columns into a single summary row.\n\n• Aggregation: By using SUM(sale_amount), the database adds up all values for that group.\n• Granularity: GROUP BY product_id, sale_date ensures that the \"granularity\" of the result set is one row per unique product/date combination.\n\nAnalysis of Incorrect Options:\n• Option A performs ranking with ROW_NUMBER() which does not aggregate; nor does it combine multiple rows into one. You would still see every individual sale record.\n• Option C uses a Window Function (OVER) which calculations perform across a set of rows but do not collapse the rows. This specific syntax would create a \"running total\" across the entire table.\n• Option D is a variation of the analyst's original query. While PARTITION BY calculates the sum for each product, the use of OVER ensures that the detail-level rows remain. It fails the requirement of producing \"exactly one row per product per day.\"",
        "domain": "Executing Queries"
    },
    {
        "id": "db-da-247",
    ...
    },
    {
        ...
        "explanation": "This is the recommended and supported workflow for configuring Genie guidance. By navigating to Configure → Context → SQL Queries, the analyst can explicitly define sample natural-language questions alongside their corresponding SQL queries. Genie uses these examples as grounding context to better understand user intent and generate accurate SQL. Reviewing and resolving inconsistencies ensures the guidance remains clear and aligned with the underlying data model, directly improving Genie’s accuracy and user experience.\n\n• Option B is incorrect: Genie does not consume notebooks as a source of sample questions or SQL guidance. Linking notebooks adds unnecessary complexity and does not integrate with Genie’s natural language understanding or context configuration mechanisms.\n\n• Option C is incorrect: The Description field is meant for human-readable documentation, not structured guidance. Genie does not parse free-form descriptive text to extract sample questions or SQL mappings, so this approach will not improve accuracy.\n\n• Option D is incorrect: This is a manual, unsupported workflow that introduces operational overhead and risk of inconsistency. Genie provides built-in configuration tools for managing sample questions and SQL examples, making external spreadsheets unnecessary and ineffective.",
        "domain": "Executing Queries"
    },
    {
        "id": "db-da-249",
        ...
        "explanation": "Lakeflow Spark Declarative Pipelines (formerly known as Delta Live Tables or DLT) are specifically designed for building reliable, production-grade data pipelines. They allow teams to define transformations declaratively using SQL or Python while embedding data quality expectations directly into the pipeline logic (such as EXPECT, EXPECT_OR_DROP, and EXPECT_OR_FAIL). The framework automatically manages execution, dependency ordering, retries, error handling, and state, removing the need for manual orchestration.\n\n• Lakeflow Jobs (A) focus on orchestration and scheduling of notebooks, scripts, or pipelines. While useful for coordinating workloads, they do not provide declarative transformation logic or built-in data quality enforcement.\n\n• Lakeflow Tables (B) are used for managing and materializing datasets but do not provide end-to-end pipeline execution, error handling, or automatic data quality enforcement.\n\n• Lakeflow Connect (D) is intended for data ingestion and connectivity from external systems. It does not support declarative transformations or automated data quality checks within pipelines.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-250",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst is evaluating options to optimize query performance on a large table with frequently changing query filters. The analyst is considering using Liquid Clustering versus traditional Hive-style partitioning. \n\nWhat is the primary advantage of Liquid Clustering over traditional partitioning for tables with evolving query patterns?",
        "options": [
            {
                "id": "a",
                "text": "Liquid Clustering allows changing clustering columns without rewriting existing data, while partitioning requires reprocessing and rewriting data to change the partitioning structure."
        ...
        "explanation": "Liquid Clustering is designed to be flexible and adaptive. It allows teams to modify clustering columns as query patterns change without having to rewrite or repartition existing data. Databricks incrementally optimizes new and existing files in the background, minimizing operational overhead. In contrast, traditional Hive-style partitioning requires a fixed partitioning scheme; changing partition columns typically involves reprocessing and rewriting the entire dataset, which is expensive and disruptive for large tables.\n\n• Option B is incorrect: Both mechanisms operate on standard column data types. Support for nested structures is not a distinguishing advantage of Liquid Clustering.\n\n• Option C is incorrect: Compression is determined by the file format (Parquet) and encoding strategies, not by the clustering or partitioning method.\n\n• Option D is incorrect: Data protection is handled by Delta Lake features like time travel and transaction logs, not by clustering mechanisms.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-251",
        ...
        "explanation": "Databricks dashboards are designed for production analytics and include native scheduling and refresh capabilities. By configuring a refresh schedule, the queries underlying the dashboard are automatically re-executed at defined intervals (for example, hourly or daily). This ensures that dashboards always display the latest data available in the underlying tables. Because the refresh is handled entirely by Databricks, it is reliable, scalable, and requires no user involvement.\n\n• Option A is incorrect: Exporting to PDF produces a static snapshot that does not update. Managers would soon be viewing outdated information.\n\n• Option C is incorrect: Using a browser extension is an unsupported and unreliable workaround that depends on individual user environments and does not guarantee that underlying queries are re-executed.\n\n• Option D is incorrect: Requiring manual refresh defeats the purpose of automation and introduces inconsistency across teams.",
        "domain": "Executing Queries"
    },
    {
        "id": "db-da-252",
        ...
        "lang": "en",
        "type": "multiple_choice",
        "prompt": "A data analyst is configuring a Genie space to support natural language questions about product sales, including fields like region_code, sku, and channel_type. The analyst notices that Genie often generates ambiguous or incorrect queries due to unclear column meanings.\n\nWhich two actions will help to improve Genie's accuracy and relevance when interpreting user questions? (Choose 2 answers)",
        "options": [
            {
                "id": "a",
                "text": "Provide sample values for ambiguous columns to help Genie infer the data's meaning and usage."
        ...
        "explanation": "To improve Genie's accuracy at the column level, two specific actions are highly effective:\n\n• Unity Catalog Column Descriptions (D): These are a primary source of semantic context. By documenting exactly what region_code or channel_type represents, Genie can more accurately map natural language terms to the correct technical fields, significantly reducing SQL generation errors.\n\n• Example Queries (E): Providing sample questions and their matching SQL queries acts as 'grounding' for Genie. It helps the model learn how business terminology (like \"online sales\") connects to the underlying database schema and logic.\n\nAnalysis of Incorrect Options:\n• Option A: While sample values can help, they are far less effective than explicit documentation and examples for conveying business meaning.\n• Option B: High-level instructions help orient users but are less impactful for resolving specific column-level ambiguity than structured metadata.\n• Option C: Adding unnecessary columns increases schema complexity and can actually make Genie's interpretations worse by introducing noise.",
        "domain": "Executing Queries"
    },
    {
        "id": "db-da-253",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst filters rows where the tags array includes the value 'sql' using this query:\n\nSELECT * FROM main.analytics.articles WHERE tags = 'sql';\n\nThis query returns no results. How should the analyst query to filter for rows where the tags array contains 'sql'?",
        "options": [
            {
                "id": "a",
                "text": "SELECT * FROM main.analytics.articles WHERE tags LIKE '%sql%';"
            ...
            {
                "id": "d",
                "text": "SELECT * FROM main.analytics.articles WHERE array_contains(tags, 'sql');"
            }
        ],
        "correctIds": [
            "d"
        ],
        "explanation": "The array_contains function is the correct and supported way to check whether a specific value exists within an array column in Databricks SQL and Spark SQL. Since tags is an array, comparing it directly to a scalar value (tags = 'sql') fails because the data types do not match.\n\n• Option A is incorrect: The LIKE operator works on string columns, not arrays.\n\n• Option B is incorrect: The IN operator checks whether a scalar value matches one of several values in a list. It does not check for membership within an array column in this context.\n\n• Option C is incorrect: Although some SQL dialects might support this syntax, Spark SQL / Databricks SQL requires the explicit use of array_contains for array membership checks.",
        "domain": "Executing Queries"
    },
    {
        "id": "db-da-254",
        ...
        "explanation": "Unity Catalog provides system-defined tags, including system.certification_status, to clearly communicate the lifecycle status of data assets such as tables and views. Setting this tag to 'deprecated' is the official and supported method to mark a table as no longer recommended for use. This status is visible across Databricks interfaces for better governance and discoverability.\n\n• Option A is incorrect: Table properties (TBLPROPERTIES) are generic metadata and are not interpreted by the Unity Catalog UI/governance workflows as lifecycle indicators.\n\n• Option B is incorrect: Adding a comment provides free-text documentation but does not formally mark the table's state in the governance framework.\n\n• Option D is incorrect: While custom tags are supported, using an arbitrary tag like 'deprecated'='true' does not integrate with the built-in system certification framework.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-255",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst is setting up a new AI/BI Genie space in Databricks to support natural language querying and ensure high-quality, governed responses. \n\nWhich component is essential for configuring an effective Genie space that returns accurate and useful results?",
        "options": [
            {
                "id": "a",
                "text": "Well-annotated datasets registered to Unity Catalog, curated sample queries, and domain-specific instructions to guide natural language translation."
        ...
        "explanation": "An effective Genie space depends on strong semantic context and governance. Registering datasets in Unity Catalog with clear table and column descriptions provides Genie with authoritative metadata. Adding curated sample questions and example SQL queries helps Genie understand how users naturally ask questions and how those questions map to the data. Domain-specific instructions further clarify business terminology and analytical intent.\n\n• Option B is incorrect: Genie is a native Databricks capability and does not require external chatbot applications.\n\n• Option C is incorrect: Genie runs on Databricks-managed infrastructure; users do not provision physical servers.\n\n• Option D is incorrect: Genie works directly with data governed by Unity Catalog in Databricks; replicating data externally is unnecessary and introduces risks.",
        "domain": "Executing Queries"
    },
    {
        "id": "db-da-256",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst wants to implement a use case that requires managing both structured and unstructured data. \n\nHow will Unity Catalog help in this scenario?",
        "options": [
            {
                "id": "a",
                "text": "It allows for the management of both structured data and unstructured data through objects such as tables and volumes."
        ...
        "explanation": "Unity Catalog provides a centralized governance layer that spans all data types in Databricks. Structured data is managed using familiar objects like tables and views, while unstructured and semi-structured data is governed using volumes, which act as cataloged containers for files. This unified approach allows administrators to apply consistent access controls, auditing, lineage tracking, and metadata management across both tabular datasets and raw files.\n\n• Option B is incorrect: Unity Catalog does not force unstructured data to be converted into Delta Lake tables before it can be managed. Files can remain in their native formats and still be governed via volumes.\n\n• Option C is incorrect: While cloud object storage is used under the hood, Unity Catalog abstracts file access through volumes, which provide a governed, cataloged interface rather than needing to manage raw blob storage paths.\n\n• Option D is incorrect: Unity Catalog explicitly supports non-tabular data through volumes. Excluding files would defeat its purpose as a unified governance system.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-257",
        ...
        "explanation": "In Databricks SQL Alerts, an alert shows the status TRIGGERED when its condition evaluates to true. This means the alert query ran successfully, the monitored value exceeded (or met) the defined threshold, and the alert fired as expected. This status clearly indicates that the alert condition has been met and any configured notifications have been sent.\n\n• Option B is incorrect: YES is not a valid status used by Databricks SQL Alerts. Alert states are expressed using specific, predefined statuses.\n\n• Option C is incorrect: SUCCESS typically refers to a query execution outcome, not the evaluation of an alert condition. An alert can execute successfully but still not meet its threshold.\n\n• Option D is incorrect: OK indicates that the alert ran successfully but the condition was not met. Since the threshold was exceeded in this scenario, the status would not be OK.",
        "domain": "Dashboards & Visualizations"
    },
    {
        "id": "db-da-258",
        ...
        "explanation": "The Databricks Data Intelligence Engine is the foundational component that powers AI-driven understanding across the Databricks platform. It combines metadata from Unity Catalog, query history, lineage, usage patterns, and embedded AI models to provide semantic understanding of data. This engine enables natural language interactions, intelligent query suggestions, optimization recommendations, and contextual explanations of datasets. Features such as Genie Spaces and Databricks Assistant are built on top of the Data Intelligence Engine.\n\n• Option A is incorrect: Genie Spaces provide a user-facing interface for NL querying, but they rely on the underlying Data Intelligence Engine to interpret questions.\n\n• Option C is incorrect: Unity Catalog focuses on data governance; while its metadata feeds into the Data Intelligence Engine, it doesn't provide NL querying or AI-driven recommendations on its own.\n\n• Option D is incorrect: Databricks Assistant assists with writing code and troubleshooting by leveraging the Data Intelligence Engine, but it is not the foundational engine responsible for platform-level insights.",
        "domain": "Executing Queries"
    },
    {
        "id": "db-da-259",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst needs to quickly estimate the number of unique user sessions in a dataset containing billions of records on Databricks SQL. Due to performance constraints, the analyst must ensure the query runs efficiently and delivers results with acceptable accuracy.\n\nWhat is the primary advantage of using APPROX_COUNT_DISTINCT instead of COUNT_DISTINCT in this scenario?",
        "options": [
            {
                "id": "a",
                "text": "APPROX_COUNT_DISTINCT provides exact counts, while COUNT_DISTINCT only provides estimates"
        ...
        "explanation": "APPROX_COUNT_DISTINCT is optimized for large-scale analytics where speed is more critical than 100% precision. It uses the HyperLogLog++ (HLL++) algorithm, which estimates the number of distinct values while requiring significantly less memory and computation than providing an exact count. By default, it provides a relative standard deviation of approximately 5%, which is sufficient for many analytical use cases.\n\n• Option A is incorrect: APPROX_COUNT_DISTINCT does not provide exact results; it intentionally trades precision for performance.\n\n• Option B is incorrect: APPROX_COUNT_DISTINCT works with multiple data types, including numeric and string columns.\n\n• Option D is incorrect: The function does not store raw values or a fixed number of unique entries. It uses probabilistic data structures (HLL++) to estimate cardinality efficiently.",
        "domain": "Executing Queries"
    },
    {
        "id": "db-da-260",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst has noticed that a key dashboard is loading slowly and suspects a specific SQL query may be causing performance issues. To diagnose the root cause, the analyst wants to review query execution characteristics such as execution time, data scanned, and rows processed over time.\n\nHow should the analyst investigate query performance over time?",
        "options": [
            {
                "id": "a",
                "text": "Use the SQL Dashboard Editor to identify slow-performing queries by reviewing visualization types and layout."
        ...
        "explanation": "Query Insights is specifically designed for tracking and analyzing query performance over time. It provides access to historical metrics such as execution duration, rows read, and data scanned across multiple runs. This allows analysts to identify trends, spikes, or regressions in specific queries that might be impacting dashboard performance.\n\n• Option A is incorrect: The Dashboard Editor is for layout and data display, not for diagnosing technical query performance.\n\n• Option B is incorrect: The Query Profiler provides a detailed breakdown of a SINGLE query execution (joins, stages, etc.). It is not designed for historical trend analysis across multiple executions.\n\n• Option D is incorrect: Lakehouse Monitoring focuses on operational monitoring of pipelines and data quality in Unity Catalog, rather than the performance of individual SQL queries.",
        "domain": "Dashboards & Visualizations"
    },
    {
        "id": "db-da-261",
        ...
        "explanation": "Lakeflow pipelines (which leverage the declarative logic of Delta Live Tables) combined with Auto Loader provide the ideal solution for this scenario. They support multi-language development (SQL and Python), are designed for automated ETL workflows, and natively handle schema evolution as new data is ingested. This automation reduces the complexity of managing schema changes across multiple sources manually.\n\n• Option A is incorrect: Fixed schemas prevent the pipeline from automatically adapting to new columns or data type changes.\n\n• Option B is incorrect: Manual scripts with cron scheduling impose a high maintenance burden and do not provide built-in automated schema evolution.\n\n• Option C is incorrect: While Workflows can schedule notebooks, manual schema management means the pipeline won't automatically handle changes, failing a key requirement of the scenario.",
        "domain": "Executing Queries"
    },
    {
        "id": "db-da-262",
        ...
        "explanation": "Marking commonly used queries as Trusted Assets is the official way to signal to users that these queries have been formally reviewed and verified for accuracy. This step maximizes trust in the Genie space, as the technical team essentially 'certifies' the logic used to derive frequently requested insights, encouraging safe reuse and consistent results across the department.\n\n• Option A is incorrect: Domain instructions are essential for context and accuracy, not a source of 'bias' to be removed.\n\n• Option B is incorrect: Automatic generation without review can lead to halucinations and decreased trust.\n\n• Option C is incorrect: Users should generally not edit the underlying datasets as it compromises data integrity and security standards.",
        "domain": "Executing Queries"
    },
    {
        "id": "db-da-263",
        ...
        "explanation": "The Databricks workspace search bar supports specific metadata operators to filter search results efficiently. The 'tag:' operator (followed by the tag key) is the officially supported syntax to find objects that have a specific tag assigned in Unity Catalog. Using 'tag:confidential' will return all tables and objects associated with that tag key.\n\n• Option A is incorrect: The search bar does not evaluate key-value pairs using boolean expressions like 'key:true'.\n\n• Option B is incorrect: Without the 'tag:' prefix, the search bar treats the input as plain text and will not correctly match metadata tags.\n\n• Option D is incorrect: The search interface does not support SQL-style comparison operators or quoted expressions.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-264",
        ...
        "explanation": "'Memory peak per operator' is the most relevant metric for investigating queries that abort due to resource exhaustion (OOM). This metric displays the maximum amount of memory consumed by each operator during execution. By pinpointing the specific operator with the highest memory peak, an analyst can identify the root cause of the cluster failure.\n\n• Option A is incorrect: Shuffle read size measures data movement, but high shuffle doesn't strictly mean memory limits are being exceeded.\n\n• Option B is incorrect: Time spent indicates slow execution but does not reveal memory pressure or resource exhaustion.\n\n• Option D is incorrect: Bytes spilled to disk is a symptom of memory pressure (spilling avoids OOM), but it is a secondary effect. When a query actually aborts due to resource constraints, checking the peak memory shows the specific limit being hit.",
        "domain": "Dashboards & Visualizations"
    },
    {
        "id": "db-da-265",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst needs to improve the performance of a daily sales report that aggregates data from a large transactions table. The report is run multiple times a day, but the underlying data only changes once daily. Which approach should the analyst use to improve performance while minimizing redundant computation?",
        "options": [
            {
                "id": "a",
                "text": "Create a materialized view that precomputes the daily sales aggregation."
        ...
        "explanation": "Materialized Views (MV) are specifically designed to precompute and persist results for expensive queries, such as large-scale aggregations. Since the underlying data only changes once per day but the report is queried multiple times, a Materialized View will store the aggregated output physically. This allows each report execution to read the precomputed results instantly without re-running the aggregation logic, significantly reducing computation costs and improving performance.\n\n• Option B is incorrect: Dynamic views (and standard views) recompute the aggregation logic every time a query is run, leading to high redundant computation overhead.\n\n• Option C is incorrect: Caching in a notebook is a manual, non-persistent, and less reliable method compared to the native governance and persistence of Materialized Views.\n\n• Option D is incorrect: Streaming tables are used for incremental ingestion of data as it arrives. Since the data only changes daily, streaming adds unnecessary architectural complexity without providing performance benefits for this batch pattern.",
        "domain": "Dashboards & Visualizations"
    },
    {
        "id": "db-da-266",
        ...
        "explanation": "Databricks notebooks provide native, built-in support for creating visualizations directly from the results of a query (using %sql) or the display() function. After running a cell, the analyst can click the '+' icon in the result interface to select 'Visualization', allowing for quick creation of line charts, bar charts, and more without leaving the notebook or writing complex plotting code.\n\n• Option A is incorrect: Exporting to Excel is inefficient and unnecessary for exploratory analysis within Databricks.\n\n• Option B is incorrect: While Plotly can be used, it adds complexity compared to the native, point-and-click visualization interface built into notebooks.\n\n• Option D is incorrect: New dashboards are typically used for sharing finalized reports, whereas the '+' icon in the notebook is the intended workflow for interactive exploration.",
        "domain": "Dashboards & Visualizations"
    },
    {
        "id": "db-da-267",
        ...
        "explanation": "In Databricks Unity Catalog, access follows a mandatory hierarchical model: Catalog → Schema → Table. To query a table, a user must have permissions at every level. Even if a user has SELECT on the table and USAGE on the schema, they will be blocked if they lack 'USE CATALOG' on the parent catalog. This is a common requirement for ensuring that users have visibility into the containers holding the data objects.\n\n• Option A is incorrect: MANAGE TABLE is an administrative privilege for metadata changes, not a requirement for reading data.\n\n• Option B is incorrect: MANAGE DATABASE provides excessive administrative control and is not required for standard object access.\n\n• Option D is incorrect: Granting ownership is an extreme measure that transfers full control and is not the standard way to resolve hierarchical access issues.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-268",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst team is working with large datasets stored as Delta tables (Parquet format) in Databricks on Azure Data Lake. The team wants to speed up query performance and reduce repeated computation by leveraging caching. However, they notice that sometimes caching does not improve performance or even slows down queries. The team wants to understand the best practices and limitations of caching in Databricks to optimize their workflows. What is the limitation when using caching in Databricks to reduce development time and query latency?",
        "options": [
            {
                "id": "a",
                "text": "Query result caching in Databricks SQL works best with non-deterministic queries that use functions like NOW() or RAND()."
        ...
        "explanation": "Delta Cache (now known as Disk Cache) is designed to accelerate read performance by caching remote cloud data (Parquet format) onto the local SSDs of worker nodes. Key limitations include being format-specific (only Parquet/Delta) and storage-specific (Cloud Object Storage). It does not apply to other formats like CSV or JSON, nor does it cache non-deterministic query results.\n\n• Option A is incorrect: Result caching only works for deterministic queries; functions like NOW() prevent caching.\n\n• Option B is incorrect: Delta Cache does not support CSV or JSON files.\n\n• Option D is incorrect: Spark caching (using memory) should be used selectively. Liberal use can lead to memory pressure and actually degrade cluster performance.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-269",
        ...
        "explanation": "Lakehouse Federation allows Databricks to query data residing in external databases (like MySQL, PostgreSQL, or SQL Server) directly without the need for ingestion. By creating a 'foreign catalog' in Unity Catalog, the external tables become visible and governable just like native Delta tables. This enables efficient joins entirely within Databricks while maintaining centralized permissions, auditing, and end-to-end lineage.\n\n• Option A is incorrect: While valid, full ingestion causes unnecessary data duplication and requires ongoing ETL maintenance, which is less efficient than federation for many use cases.\n\n• Option B is incorrect: Temporary JDBC views are not managed by Unity Catalog, so you lose the required governance, auditing, and lineage enforcement.\n\n• Option D is incorrect: Joining outside Databricks bypasses the platform's security and compute benefits entirely.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-270",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst encounters an unclear error while running a SQL query in the Databricks SQL Editor. The analyst wants to quickly understand and resolve the issue. How can Databricks Assistant help the analyst diagnose and fix the error?",
        "options": [
            {
                "id": "a",
                "text": "Databricks Assistant requires switching to a notebook environment to provide any debugging support."
        ...
        "explanation": "Databricks Assistant is an AI-powered sidekick integrated directly into the SQL Editor and notebooks. It is designed to interpret SQL errors, explain potential root causes, and suggest specific code fixes or optimizations. This allows analysts to troubleshoot complex issues efficiently without leaving their current workspace or manually searching for error documentation.\n\n• Option A is incorrect: The Assistant works natively within the SQL Editor.\n\n• Option B is incorrect: It supports multiple languages, including SQL, Python, and R.\n\n• Option D is incorrect: Debugging and error analysis are core features of the Assistant, not restricted to just creating new content.",
        "domain": "Executing Queries"
    },
    {
        "id": "db-da-271",
        ...
        "explanation": "'DESCRIBE HISTORY' is the fundamental Delta Lake command used to view the table's transaction log. It provides a detailed audit trail of every operation (INSERT, UPDATE, DELETE, MERGE, etc.), including timestamps, user identity, and operations performed. This information is essential for auditing and for time travel operations.\n\n• Option B is incorrect: Delta Lake does not store history as a queryable column within the data table itself.\n\n• Option C is incorrect: VACUUM is a maintenance command used to remove old data files that are no longer referenced in the transaction log; it does not display history.\n\n• Option D is incorrect: 'SHOW TRANSACTIONS' is not a valid SQL command in Databricks for viewing table-level history.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-272",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst has configured a Delta table with default retention settings. The analyst runs VACUUM operations weekly to manage storage costs. A business user asks to access data from 10 days ago for a compliance audit. What is the result of a time travel query in this scenario?",
        "options": [
            {
                "id": "a",
                "text": "The time travel query will fail because the default maximum table version is 7 and numbering resets after each VACUUM operation."
            },
            {
                "id": "b",
                "text": "The time travel query will succeed because the default log retention period is 30 days, which is sufficient to access 10-day-old data."
        ...
        "explanation": "To perform time travel, Delta Lake requires two things: the transaction log entries (retained for 30 days by default) and the actual underlying data files. Since the default 'VACUUM' retention period is 7 days (168 hours), running a weekly VACUUM operation will permanently delete data files older than 7 days. Even if the log entries for 10 days ago still exist, the physical files are gone, causing the time travel query to fail.\n\n• Option A is incorrect: Version numbers do not reset after VACUUM, nor is there a 'maximum version of 7' limit.\n\n• Option B is incorrect: Log retention is 30 days, but without the physical files (removed by VACUUM), the logs alone cannot reconstruct the data.\n\n• Option D is incorrect: Delta Lake does not perform automatic backups outside of the configured retention settings.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-273",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst is reviewing a Genie space used by operations managers and notices that multiple responses have been flagged as inaccurate. Which action should the analyst take to improve performance?",
        "options": [
            {
                "id": "a",
                "text": "Add more tables and columns to the space because increasing the data volume improves Genie's ability to find the correct answers."
        ...
        "explanation": "To improve the performance and accuracy of an AI/BI Genie space, analysts should provide clear instructions and representative 'example SQL queries'. These examples serve as patterns that help the model understand the business context, naming conventions, and complex join logic required for a specific domain. Better context leads to better generated SQL.\n\n• Option A is incorrect: Adding excessive or irrelevant tables increases noise and ambiguity, often decreasing performance.\n\n• Option C is incorrect: Feedback is a critical tool for identifying gaps; turning it off would prevent improvement.\n\n• Option D is incorrect: Simply deleting flags does not fix the underlying cause (lack of context).",
        "domain": "AI/BI Genie Spaces"
    },
    {
        "id": "db-da-274",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst needs to create a table in Unity Catalog for a production analytics workload. The table will be frequently queried and needs automatic performance optimization. The data should be governed by Unity Catalog with lifecycle management. Which approach should the data analyst use?",
        "options": [
            {
                "id": "a",
                "text": "Create an external table using CREATE EXTERNAL TABLE catalog.schema.table_name LOCATION 's3://bucket/path/'"
            },
            {
                "id": "b",
                "text": "Create a temporary table using CREATE TEMPORARY TABLE for better performance and then convert it to a managed table later using ALTER TABLE SET MANAGED."
            ...
            {
                "id": "d",
                "text": "Create a foreign table using CREATE FOREIGN TABLE with external connections to maintain compatibility with external systems."
            }
        ],
        "correctIds": [
            "c"
        ],
        "explanation": "Managed tables are the recommended approach for production workloads in Databricks where 'hands-off' optimization is required. Unity Catalog fully manages the data lifecycle and storage location. Critically, features like 'Predictive Optimization' (which automatically handles COMPACT and VACUUM) and advanced performance tuning are natively supported for managed tables.\n\n• Option A is incorrect: External tables require manual management of the storage location and lifecycle, and have limited support for automatic optimizations.\n\n• Option B is incorrect: Temporary tables are session-scoped and not governed by Unity Catalog.\n\n• Option D is incorrect: Foreign tables are for federation and do not support the internal optimizations found in managed Delta tables.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-275",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst is monitoring delivery delays in real time using a Databricks dashboard shared with the operations team. After a meeting, the manager asks to add a new widget that graphically displays, with a gauge, the percentage of on-time shipments compared to the total. The data analyst needs to quickly modify the dashboard to add this information element. How should the analyst add a new widget to an existing dashboard in Databricks?",
        "options": [
            {
                "id": "a",
                "text": "Ask a platform administrator to add a materialized view for the on-time percentage, since visualizations require pre-aggregated values from the source to display correctly."
            ...
            {
                "id": "d",
                "text": "Clone the existing dashboard, and add the new visualization in the duplicate, and replace the original dashboard after review."
            }
        ],
        "correctIds": [
            "c"
        ],
        "explanation": "Databricks dashboards provide an intuitive, self-service interface for modifying and creating visualizations. Any analyst with edit permissions can directly add a new widget by clicking 'Add Visualization', choosing the appropriate chart type (like a Gauge chart), and linking it to a SQL query that performs the necessary calculation. This allows for rapid iteration and real-time updates as underlying data changes.\n\n• Option A is incorrect: Materialized views are not required for dashboard visualizations; any standard SQL query result can be visualized.\n\n• Option B is incorrect: Static images do not update and lack interactivity, making them unsuitable for real-time monitoring.\n\n• Option D is incorrect: While cloning is possible, it is an unnecessarily complex and redundant process for simply adding a single widget.",
        "domain": "AI/BI Genie Spaces"
    },
    {
        "id": "db-da-276",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst needs to programmatically mark a Unity Catalog table, sales_data, as certified so that it is discoverable as a trusted asset across their organization. Which SQL command can be used to meet the requirement?",
        "options": [
            {
                "id": "a",
                "text": "ALTER TABLE sales_data SET TAGS ('certified' = 'true');"
        ...
        "explanation": "In Unity Catalog, certification and tagging are used to organize and identify trusted assets. Certification is implemented by adding specific tags to tables. The correct SQL syntax for setting a tag is 'ALTER TABLE ... SET TAGS' using key-value pairs enclosed in parentheses. This makes the table easily discoverable as a 'trusted' asset in Catalog Explorer and search results.\n\n• Option B is incorrect: Tags must be specified as key-value pairs (e.g., 'key' = 'value'). Providing only a key without a value leads to a syntax error.\n\n• Option C is incorrect: There is no 'CERTIFY' privilege in Databricks' SQL access control.\n\n• Option D is incorrect: While comments provide descriptions, they do not trigger the 'certified' metadata badge or discovery features associated with tags.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-277",
        ...
        "explanation": "Databricks Auto Loader is the purpose-built solution for streaming file ingestion from cloud object storage (S3, ADLS, GCS). It efficiently tracks new files, supports incremental processing (exactly once), and provides built-in mechanisms for schema inference and schema evolution. This abstracts away the complexity of manual file tracking and simplifies handling semi-structured data like logs.\n\n• Option A is incorrect: While related, a 'basic' custom streaming job requires more manual effort for file tracking and schema management compared to the optimizations in Auto Loader.\n\n• Option B is incorrect: S3 is object storage, not an API source that requires custom pull logic for streaming.\n\n• Option C is incorrect: Delta Sharing is for cross-organization data sharing, not for internal raw data ingestion.",
        "domain": "Importing Data"
    },
    {
        "id": "db-da-278",
        ...
        "explanation": "Databricks SQL support 'Query Parameters' using the colon syntax (e.g., :parameter_name). In the SQL Editor, these markers automatically generate interactive input boxes above the results pane. This allows analysts to test queries with different values dynamically without modifying the code, which is the standard workflow before publishing the query to a dashboard.\n\n• Option B is incorrect: Standalone SQL Editor queries cannot directly reference Python variables.\n\n• Option C is incorrect: Databricks SQL requires named parameters (e.g., :param) rather than positional question marks (?).\n\n• Option D is incorrect: Hard-coding values is inefficient and defeats the purpose of creating reusable, parameterized dashboard queries.",
        "domain": "Executing Queries"
    },
    {
        "id": "db-da-279",
        ...
        "lang": "en",
        "type": "single_choice",
        "prompt": "A data analyst discovers that a group named developers has unauthorized access to sensitive data within the Unity Catalog catalog finance_catalog, which should be accessible only by the finance team. Which SQL command should the analyst use to remove the developers group's access to the catalog?",
        "options": [
            {
                "id": "a",
                "text": "ALTER CATALOG 'finance_catalog' DROP PERMISSIONS FOR 'developers';"
        ...
        "explanation": "In Unity Catalog, the 'USAGE' privilege is mandatory for users to see and interact with objects inside a catalog or schema. To remove access, the 'REVOKE' statement must be used with the 'USAGE' privilege. Revoking 'USAGE' on a catalog effectively blocks the specified group from accessing any schemas or tables within that catalog.\n\n• Option A is incorrect: 'ALTER CATALOG ... DROP PERMISSIONS' is not valid Delta or Unity Catalog syntax.\n\n• Option C is incorrect: Granting access to one group does not automatically remove access from another.\n\n• Option D is incorrect: 'USE' is not a valid privilege in Unity Catalog; 'USAGE' is the correct term.",
        "domain": "Managing Data"
    },
    {
        "id": "db-da-280",
